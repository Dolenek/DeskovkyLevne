    ...
    {items.map((item) => {
      ...
      return (
        <button
          ...
          className={`flex min-h-[46px] items-center flex-wrap justify-between gap-x-2 gap-y-1 rounded-md border px-2 py-2 text-left transition ${
            item.isActive
        ...
        >
          <span className="flex min-w-0 items-center gap-2">
            ...
            <span className="min-w-0">
              <span className="block break-words text-xs sm:text-sm font-extrabold">{item.label}</span>
              <span className="hidden text-xs font-bold text-muted sm:block">
                {item.isBest ? chartCopy(locale, "detailChartCheapest") : (difference ?? "\u00a0")}
              </span>
            </span>
          </span>
          <span className="shrink-0 text-right text-xs sm:text-sm font-extrabold text-primary">{price}</span>
        </button>
      );
    })}
  </div>
