...
test("filter metadata comes from API and reset clears active filters", async ({ page }) => {
  ...
  await page.route("**/api/v1/meta/price-range**", async (route) => {
    ...
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: '{"min_price":149,"max_price":1999}',
    });
  });
  ...
  await expect(page.getByRole("banner").getByRole("link", { name: "E-shopy" })).toHaveCount(0);
  ...
  await expect(page.getByText("3 e-shopy", { exact: true })).toBeVisible();
  await expect(page.locator("main svg.lucide-star")).toHaveCount(0);
  await page
    .getByRole("button", { name: /Strategick/ })
    .first()
    .click();
  await expect(page.getByRole("button", { name: "Odebrat filtr Strategická" })).toBeVisible();
...
test("manual price filters normalize negative and reversed bounds", async ({ page }) => {
  ...
  await expect
    .poll(() =>
      catalogUrls.some((requestUrl) => {
        const url = new URL(requestUrl);
        return url.searchParams.get("min_price") === "500" && url.searchParams.get("max_price") === "1200";
      }),
    )
    .toBe(true);
